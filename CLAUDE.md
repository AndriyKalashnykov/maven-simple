...
- [ ] **CRITICAL — Renovate is NOT running (verified 2026-06-29).** Last `app/renovate` PR was #337 on 2026-04-01; the Dependency Dashboard (issue #287) froze at 2026-04-04; zero `renovate/*` branches exist while ~9 routine updates are available. The Mend GitHub App appears uninstalled/suspended — dependency bumps since April have been landing **manually** via `/project-review` and `/ship-it` sessions, not the bot. **Fix is external** (cannot be done from the repo): reinstall/reactivate at <https://github.com/apps/renovate> and check repo status at developer.mend.io. **Until reactivated, all `renovate.json` config is inert** (incl. `platformAutomerge:false`, the `pinDigests:false` rule, the renovate self-throttle, and all automerge/grouping) and every "Renovate will bump it" note below is on hold — bump manually. The 2026-06-29 drop-in backlog was applied manually via `/ship-it` (jackson 3.2.0, junit 6.1.1, okhttp 5.4.0, trivy 0.71.2, act 0.2.89, maven 3.9.16, mermaid-cli 11.15.0, dependency-check 12.2.2, jacoco 0.8.15, enforcer 3.6.3, maven-dependency-plugin 3.11.0, plus GH Actions checkout v7 / cache v6 / mise-action v4.2.0); these manual sweeps must be repeated each session until the Mend app is reactivated. Still held (pre-release, not drop-in): maven-compiler-plugin 4.0.0-beta, maven-resources-plugin 4.0.0-beta, SLF4J 2.1.0-alpha, WireMock 4.0-beta, jackson-annotations 3.0-rc; Java stays 25 LTS (26 is non-LTS).
